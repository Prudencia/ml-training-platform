...
cJSON* Model_Setup(void) {
    ...
    // Read confidence threshold from model.json (if present, otherwise keep default)
    cJSON* confItem = cJSON_GetObjectItem(modelConfig, "confidence");
    if (confItem) {
        confidenceThreshold = confItem->valuedouble;
    }

    LOG_TRACE("Boxes: %d Classes: %d Objectness: %f Confidence: %f NMS: %f", boxes, classes, objectnessThreshold, confidenceThreshold, nms);
